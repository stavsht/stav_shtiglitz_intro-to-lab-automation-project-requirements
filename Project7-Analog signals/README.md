...
- test your code.
- View in scope: Connect Analog 2 (dark blue) to pin 4 in arduino (LED output). Play with the times and triggers until you see the PWM change when turning the rotary. ![alt text](ADALM.PNG)
- View in voltmeter - stop scope first. See the average voltage change. What is the duty cycle? the relation between the time that its on and off
- what happens when using 30Hz instead of 50Hz for the PWM?  we decresed the freq so we see less cycles for the same period of time. 
...
- How does the range of the servo motor compare to the range of the rotary? Use the scope. more narrow
- is the range of our server the same as the range of the package? change the range of values to the servo accordingly.  the range of the package is 0 to 180. degrees. our servo can't reach 180 (its seems like ~160 is the max)
